...
from ops.feature_loader import BasicDataset, ResamplingDataset_Mask, LTPlusDataset
...
def load_data(num_class, input_dir, dataset='VideoLT'):
    ...
    if args.resample == 'None':
        if dataset == 'Charades' or dataset == 'CharadesEgo':
            ...
            train_dataset = LTPlusDataset(train_list, input_dir, args.train_num_frames, cls_num=args.num_class, train_mode=True)
    else:
    ...
    if dataset == 'Charades' or dataset == 'CharadesEgo':
        ...
        val_dataset = LTPlusDataset(val_list, input_dir, args.val_num_frames, \
            cls_num=args.num_class, train_mode=False)
...
def train(loader, model, epoch, log, indices, ap=0, ap1=0):
    ...
    for i, (vid, feature, target) in enumerate(loader):
        ...
        feature = feature.float().to(args.local_rank)

        target = target.float().to(args.local_rank)

        if epoch <= args.warm_epoch:
            prediction, output, z, prediction1, output1= model(feature, phase='val')
            ...
            prediction, output, z, prediction1, output1 = model(mixed_input, phase='val')
