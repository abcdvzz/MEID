...
class ResamplingDataset_Mask(Dataset):
    def __init__(self, data_list, input_dir, num_frames, rstype, cls_num=1004, train_mode=True):
        ...
        for data in self.data_list:
            vid_id, label, lt_label = data.split()
            # vid_id, label = data.split()
...
class LTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     [int(i) for i in label.split(',')]
        # ], axis=0)
        label = np.array([float(i) for i in label.split(',')])
...
class LTPlusDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # label = np.array([float(i) for i in label.split(',')])
        # lt_label = [int(i) for i in lt_label.split('-')]
        # lt_label = [float(i) for i in lt_label.split('-')]
        # lt_label = np.array(lt_label)

        # return vid_id, feature, label, lt_label
        return vid_id, feature, label
...
class LTPlusDataset_diff(Dataset):
    def __init__(self, data_list, input_dir, num_frames, cls_num=1004, train_mode=True):
        self.data_list = data_list
        self.input_dir = input_dir
        self.num_frames = num_frames
    ...
    def __getitem__(self, index):
        ...
        vid_id, label, lt_label = data.split()
        feature = np.load(os.path.join(self.input_dir, '%s.npy'%(vid_id)))
        # feature_diff = np.load(os.path.join(self.input_dir, '%s_diff.npy'%(vid_id)))
        # feature_diff = np.load(os.path.join(self.input_dir, '%s_diff_0.npy'%(vid_id)))
        # feature_diff = np.load(os.path.join(self.input_dir, '%s_diff_mean.npy'%(vid_id)))
        feature_diff = np.load(os.path.join(self.input_dir, '%s_diff_1.npy'%(vid_id)))
        if self.train_mode:
            feature = self.random_sample(feature, self.num_frames)
            feature_diff = self.random_sample(feature_diff, self.num_frames)
        else:
            feature = self.uniform_sample(feature, self.num_frames)
            feature_diff = self.uniform_sample(feature_diff, self.num_frames)
        ...
        # return vid_id, feature, label, lt_label
        return vid_id, feature, feature_diff, label
...
class LTPlusDataset_diff_charades(Dataset):
    def __init__(self, data_list, input_dir, num_frames, cls_num=1004, train_mode=True):
        self.data_list = data_list
        self.input_dir = input_dir
        self.num_frames = num_frames
...
class ResamplingDataset_Mask_diff(Dataset):
    def __init__(self, data_list, input_dir, num_frames, rstype, cls_num=1004, train_mode=True):
        self.data_list = data_list
        self.input_dir = input_dir
        self.num_frames = num_frames
    ...
    def __getitem__(self, index):
        ...
        return vid_id, feature, feature_diff, label, mask
...
class ResamplingDataset_Mask_diff_charades(Dataset):
    def __init__(self, data_list, input_dir, num_frames, rstype, cls_num=1004, train_mode=True):
        self.data_list = data_list
        self.input_dir = input_dir
        self.num_frames = num_frames
