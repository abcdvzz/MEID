...
from ops.feature_loader import BasicDataset, LTPlusDataset, LTPlusDataset_diff_charades
...
def test(loader, model, indices):
    ...
    with torch.no_grad():
        # for i, (vid, feature, target, lt_label) in enumerate(loader):
        for i, (vid, feature, feature_diff, target) in enumerate(loader):
            feature = feature.cuda()
            target = target.float().cuda()
            feature_diff = feature_diff.cuda()
            
            # prediction, output, z = model(feature, lt_label)
